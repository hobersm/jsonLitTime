...
def main():
    ...
    for entry in data:
        cleaned_entry = {
            ...
            "URL": entry.get("url", "URL not found"),
            "SELECT": None
        }
        if 'Fiction' in cleaned_entry["Categories"]:
            # cleaned_data.append(cleaned_entry)
            # Group entries by time
            time_key = entry.get("time", "Time not found")
            if time_key not in cleaned_data:
                cleaned_data[time_key] = []
            cleaned_data[time_key].append(cleaned_entry)
    ...
    with open(file_path, 'w') as file:
        ...
        # else:
        json.dump(cleaned_data, file, ensure_ascii=False, indent=1)
